<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
		...
		<form id="inqryFrm" name="inqryFrm" action="./action" class="inqry-form" style="margin: 0 auto; max-width:768px;" method="post" enctype="multipart/form-data">
			...
			<table class="table-detail">
				...
				<tbody>
					...
					<tr>
						<td colspan="2">
							<div class="inqry-option">
								<label for="inqryTy0">
									<input type="radio" name="inqryTy" id="inqryTy0" checked="checked" value="bsns">
									<span>사업제휴</span>
								</label>
			...
			<table class="table-detail">
				...
				<tbody>
					...
					<tr>
						<th scope="row">
							<p>
								<label for="cntntsTel">전화번호<sup class="text-danger text-base md:text-lg">*</sup></label>
							</p>
						</th>
						<td><input type="text" class="form-control w-full" id="cntntsTel" name="cntntsTel" maxlength="100" oninput="autoHyphen(this)"></td>
					</tr>
					<tr>
						<th scope="row"></th>
						<td><p class="text-sm">※ 연락 가능한 전화번호를 입력하세요.</p></td>
					</tr>
				</tbody>
			</table>
			...
			<button type="button"
					class="group flex w-full items-center justify-between text-title2 mt-10 md:mt-17"
					data-bs-target="#collapse-agree1"
					data-bs-toggle="collapse"
					aria-expanded="true">
				<span>개인정보 수집 및 이용 동의</span>
				<img src="/html/page/members/assets/images/ico-angle-down.svg"
					 alt=""
					 style="width: 14px; height: 14px;"
					 class="mt-0.5 w-[0.875em] transition-transform ease-in-out group-aria-expanded:rotate-180">
			</button>
			<div class="border-t-0.5 border-gray2 mt-2 md:mt-3">
				<div class="collapse show" id="collapse-agree1">
					<div class="mt-2 px-5 py-4 rounded-[1em] bg-neutral-100 text-sm md:mt-3 md:px-8 md:py-6 md:text-base">
						...
						<ol class="list-number mt-4 md:mt-5 spacey-y-1 md:spacey-1.5">
							...
							<li>수집하는 개인정보 항목: 회사명, 이름, E-mail, 전화번호(필수)</li>
			...
			<div class="flex justify-center space-x-2 mt-11 sm:space-x-2.5 md:mt-18">
...
<script>
	function autoHyphen(target) {
		...
		if (target.value.startsWith('02')) {
			target.value = target.value.replace(/^(\d{2})(\d{3,4})(\d{4})$/g, function(match, p1, p2, p3) {
				if (p1 === '02') return p1 + (p2 ? '-' + p2 : '') + (p3 ? '-' + p3 : '');
				return match;
			})
		} else {
			target.value = target.value.replace(/^(\d{0,3})(\d{0,4})(\d{0,4})$/g, function(match, p1, p2, p3) {
				if (p1 !== '02' && p1 !== '') return p1 + (p2 ? '-' + p2 : '') + (p3 ? '-' + p3 : '');
				return match;
			})
		}
		target.value = target.value.replace(/(-{1,2})$/g, '');
	}
	...
	$(function(){
		...
		$.validator.addMethod("isEmail", function(value, element, regexpr) {
			return value === '' || regexpr.test(value);
		}, "이메일 형식이 일치하지 않습니다. ");

		$.validator.addMethod("isTel", function(value, element, regexpr){
			return value === '' || regexpr.test(value);
		},"전화번호 형식이 일치하지 않습니다.");

		$.validator.addMethod("agreeChk", function(value,element){
			return !!$("#cntntsAgree").is(":checked");
		...
		$("#inqryFrm").validate({
		    ...
		    ,	cntntsEml : {required : true, isEmail : emailRegex}
			,	cntntsTel : {required : true, isTel : telRegex}
			...
			,	cntntsTel : {required : "전화번호를 입력해주세요."}
