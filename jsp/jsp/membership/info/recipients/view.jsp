	...
	<div id="page-content">               
		...
		<div class="mypage-client-details mt-3.5 md:mt-5">
			<div class="mypage-client-detail-item">
				<h4 class="mypage-client-detail-item-title">
					...
					<a href="#" class="text-sm text-blue3" data-bs-toggle="modal" data-bs-target="#dbapp_modal">수정하기<i class="icon-plus icon-blue3"></i></a>
				</h4>
							...
							<tr>
								...
								<td>
									<c:if test="${consltResultVO != null}">
										...
										<div class="justify-end item-request">
		...
		<div class="flex flex-col gap-4 mt-8">
            <div class="flex items-center justify-between gap-2">
                <h3 class="text-xl font-semibold">관심 복지용구</h3>
                <div>
                	<c:if test="${equipCtgryStatus eq 'consult'}">
                		<a class="btn-success btn-small" data-bs-toggle="modal" data-bs-target="#dbapp_modal">상담하기</a>
                	</c:if>
                </div>
            </div>
            ...
            <c:choose>
            	...
            	<c:otherwise>
            		<div class="card-bg-gray">
						...
						<p class="text-center text-gray5">관심 복지용구를 선택하고 <br>장기요양보험 혜택을 받아보세요</p>
						<a class="mt-8 btn-outline-secondary btn-arrow" data-bs-toggle="modal" data-bs-target="#dbapp_modal">
							선택하기
							<i class="ml-3 icon-next"></i>
						</a>
			        </div>
            	</c:otherwise>
            </c:choose>
		...
		<div class="flex items-center justify-between gap-2 mt-8">
			<h3 class="text-xl font-semibold">인정등급 예상 테스트</h3>

			<c:choose>
				<c:when test="${testStatus eq 'notgrade'}">
					<a class="btn-success btn-small" data-bs-toggle="modal" data-bs-target="#dbapp_modal">상담하기</a>
				</c:when>
				<c:when test="${testStatus eq 'consult'}">
					<a class="btn-success btn-small" data-bs-toggle="modal" data-bs-target="#dbapp_modal">상담하기</a>
				</c:when>
				<c:when test="${testStatus eq 'viewconsult'}">
				</c:when>
			</c:choose>
		...
		<c:choose>
			<c:when test="${testVO != null}">
				<div class="mt-4 card-bg-gray">
					...
					<p class="text-xs text-gray5">최근 테스트 일시 :<span><fmt:formatDate value="${testVO.mdfcnDt != null ? testVO.mdfcnDt : testVO.regDt}" pattern="yyyy년 MM월 dd일 HH:mm:ss" /></span></p>
					<a href="#" class="mt-8 btn-outline-secondary"  data-bs-toggle="modal" data-bs-target="#grade-test-result">
						결과 상세보기
						<i class="ml-3 icon-next"></i>
					</a>
				</div>
			...
			<c:otherwise>
				<div class="mt-4 card-bg-gray">
					...
					<a class="mt-8 btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#dbapp_modal">
						바로가기
						<i class="ml-3 icon-next"></i>
					</a>
					<!-- todo end -->
		...
		<div class="my-20 text-center">
			<a href="/membership/info/recipients/list" class="w-1/3 btn-primary btn-large">목록</a>
		</div>
	</div>
...
<script>
	...
	//테스트 상담하기 버튼 클릭(등급이 없을 경우)
	function fn_test_nograde_click(){
		if (!confirm("대상자가 아니에요.\n받을 수 있는 다른 혜택을 알아보시겠어요?")){
			return;
		}
