...
<link rel="stylesheet" href="/html/core/script/ckeditor5-online-builder/sample/styles.css?v=<spring:eval expression="@version['assets.version']"/>"/>
				...
				<form:form name="frmNtt" id="frmNtt" modelAttribute="nttVO" method="post" action="./action" enctype="multipart/form-data" class="form-horizontal">
					...
					<input type="hidden" id="temp2move" name="temp2move" value="" />
					<input type="hidden" id="existImages" name="existImages" value="" />
                ...
                <script>
					$("body").attr("data-editor", "ClassicEditor")
                	...
                	$("form[name='frmNtt']").validate({
                	    ...
                	    submitHandler: function (frm) {
               	            if (confirm('<spring:message code="action.confirm.save"/>')) {
								...
								var htmlContent = jsCKEditorHelper.getData();
								$("#cn").html(htmlContent);
				...
				function fn_cheditor_image_adjust(){
					var htmlContent = jsCKEditorHelper.getData();

					var ifor, ilen = $(".ck img").length;//[src*=\\/tempImage\\/]
					var temp2move = [], existImages = [];
					for(ifor=0;ifor<ilen;ifor++){

						var jobjTarget = $($(".ck img")[ifor]);
						if (jobjTarget.attr("src") != undefined){
							if (jobjTarget.attr("src").indexOf("/tempImage/") >= 0 ){
								temp2move.push(jobjTarget.attr("src"));
							}else{
								existImages.push(jobjTarget.attr("src"));
							}
						}
						
					}

					return {temp2move, existImages}
				}
