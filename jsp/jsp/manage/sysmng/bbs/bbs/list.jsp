<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
                ...
                <p class="text-title2 mt-13">
                    [${bbsSetupVO.bbsNm}] 목록
                    <c:if test="${bbsSetupVO.getNttOrdrYn() eq 'Y' }">
                        <br>
                        <button type="button" class="btn btn-light" id="sort_reload">순서 저장</button>
                    </c:if>
                ...
                <table class="table-list">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <c:if test="${bbsSetupVO.getNttOrdrYn() eq 'Y' }">
                                <td><input type="text" id="sortNo${status.index}" name="bbs_list_sort" class="form-control text-center keycontrol numberonly" value="${resultList.nttOrdr}" data-ntt-no="${resultList.nttNo}" data-bbs-no="${resultList.bbsNo}" /></td>
                            </c:if>
                            
                            <td>${listVO.startNo - status.index}</td>
                            <td class="text-left">
                            	<c:forEach begin="1" end="${resultList.nttLevel}">&nbsp;&nbsp;&nbsp;&nbsp;</c:forEach>
                                <c:if test="${bbsSetupVO.getNttOrdrYn() ne 'Y' }"><!--순번 변경이 가능한 경우는 댓글이 없다-->
                ...
                $(function(){
                    ...
                    $("#sort_reload").off("click").on("click", function(){
                        fn_bbs_list_sort_click();
                    });
                });

                function fn_bbs_list_sort_click(){

                    var jsonData = {bbsNo : "${bbsNo}"};
                    var joTarget;
                    var arrNttNo = [];
                    var arrNttSort = [];
                    var ifor, ilen = $("input[name='bbs_list_sort']").length;

                    for(ifor=0 ; ifor<ilen ; ifor++){
                        joTarget = $($("input[name='bbs_list_sort']")[ifor]);

                        arrNttNo.push(  joTarget.data("ntt-no"));
                        arrNttSort.push(  joTarget.val() );
                    }
                    jsonData.arrNttNo = arrNttNo;
                    jsonData.arrNttSort = arrNttSort;

                    if (!confirm("순번을 저장하시겠습니까?")){
                        return;
                    }
                    ...
                    jsCallApi.call_api_post_json(window, '/_mng/sysmng/bbs/list/sortChg.json', "fn_bbs_list_sort_cb", jsonData);
                }
                function fn_bbs_list_sort_cb(result, errorResult, data, param){
                    $("#searchFrm").submit();
                }
                </script>