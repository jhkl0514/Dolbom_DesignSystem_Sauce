...
<div id="page-content">
	<form action="./list" method="get" id="searchFrm" name="searchFrm">
		...
		<fieldset>
			<legend class="text-title2">브랜드 검색</legend>
			<table class="table-detail">
				...
				<tbody>
					<tr>
						<th scope="row"><label for="search-item1">브랜드명</label></th>
	...
	<p class="text-title2 mt-13">브랜드 목록</p>
		...
		<tbody>
			...
			<c:forEach var="result" items="${listVO.listObject}" varStatus="status">
				<tr>
					...
					<td>${result.brandNo}</td>
					<td><a href="./form?brandNo=${result.brandNo }&${pageParam}">${result.brandNm}</a></td>
					<td>${result.telno }</td>
