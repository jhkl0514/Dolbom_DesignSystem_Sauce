...
<div id="page-content">
	<form action="./list" method="get" id="searchFrm" name="searchFrm">
		...
		<fieldset>
			<legend class="text-title2">배송업체 검색</legend>
			<table class="table-detail">
				...
				<tbody>
					<tr>
						<th scope="row"><label for="srchText">배송업체명</label></th>
						...
						<td>
							<div class="form-check-group">
								<div class="form-check">
									<input class="form-check-input" type="radio" name="srchUseYn" id="search-item3" value="" checked> <label class="form-check-label" for="search-item3">전체</label>
								</div>
								<c:forEach var="yn" items="${useYn}">
									<div class="form-check">
										<input type="radio" value="${yn.key}" id="${yn.value}" name="srchUseYn" class="form-check-input" <c:if test="${yn.key eq param.srchUseYn}">checked="checked"</c:if>> <label class="form-check-label" for="${yn.value}">${yn.value}</label>
									</div>
								</c:forEach>
							</div>
						</td>
	...
	<p class="text-title2 mt-13">배송업체 목록</p>
	<table class="table-list">
		...
		<thead>
			<tr>
				...
				<th scope="col">안내전화번호</th>
				<th scope="col">배송추적링크</th>
				<th scope="col">상태</th>
			</tr>
		</thead>
		<tbody>
		<c:forEach var="resultList" items="${listVO.listObject}" varStatus="status">
			...
			<tr>
				...
				<td><a href="./form?coNo=${resultList.coNo}&${pageParam}">${resultList.dlvyCoNm }</a></td>
				<td>${resultList.telnoInfo }</td>
				<td>${resultList.dlvyUrl }</td>
