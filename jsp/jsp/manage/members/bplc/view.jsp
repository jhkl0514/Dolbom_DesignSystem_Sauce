...
<%@ page import="icube.common.values.CodeMap"%>
...
<script type="text/javascript" src="/html/core/script/JsHouse2309PopupJqgrid.js"></script>

<script type="text/javascript" src="/html/page/admin/assets/script/_mng/bplc/JsPopupLogDataColumnTableKeyList.js"></script>
...
<form:form name="frmBplc" id="frmBplc" modelAttribute="bplcVO" method="post" action="./action" enctype="multipart/form-data">
	...
	<fieldset class="mt-13">
		...
		<table class="table-detail join">
			...
			<tbody>
				...
				<tr>
					<th scope="row">장기요양 매칭상담 서비스<span></span></th>
					<td>
						<div class="form-check-group">
	...
	<fieldset class="mt-13">
		<legend class="text-title2 relative"> 장기요양 매칭 상담 정보 </legend>
		<table class="table-detail join">
			<colgroup>
				<col class="w-43">
				<col>
			...
			<tbody>
				<tr>
					<th scope="row"><label for="joinDt">상담 가능 유형</label></th>
					<td>
						${htmlConsltAbleType}
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="trmsAgreDt">인정등급 대리신청</label></th>
					<td>
						<div class="form-check-group">
							<c:forEach items="${CodeMap.INSTEAD_RCPERRCOGN_YN}" var="itemone" varStatus="status">
								<div class="form-check">
									<form:radiobutton class="form-check-input" path="insteadRcperrcognYn" id="insteadRcperrcognYn${status.index}" value="${itemone.key}"  disabled="true"/>
									<label class="form-check-label" for="insteadRcperrcognYn${status.index}">${itemone.value}</label>
								</div>
							</c:forEach>
						</div>
					</td>
				...
				<tr>
					<th scope="row">L번호 미보유자 처리</th>
					<td>
						<div class="form-check-group">
							<c:forEach items="${CodeMap.HANDLE_NO_RCPERRCOGN_YN}" var="itemone" varStatus="status">
								<div class="form-check">
									<form:radiobutton class="form-check-input" path="handleNoRcperrcognYn" id="handleNoRcperrcognYn${status.index}" value="${itemone.key}"  disabled="true"/>
									<label class="form-check-label" for="handleNoRcperrcognYn${status.index}">${itemone.value}</label>
								</div>
							</c:forEach>
						</div>
					</td>
				...
				<tr>
					<th scope="row">매칭 우선순위<span></span></th>
					<td>
						<form:select class="form-control" path="autoMatchingPoint" disabled="true">
							<c:forEach var="ifor" begin="1" end="10">
								<form:option value="${ifor}">${ifor}</form:option>
							</c:forEach>
	...
	<div class="btn-group right mt-8">
		<button type="button" class="btn-primary large shadow f_view_log">변경이력보기</button>
...
<div class="modal-con">
    <c:import url="/_mng/api/popups/jqgridCloseonlyModal.form?modalId=dataColumnChangedModal&title=변경이력" />
</div>


<script>

	var jsPopupLogDataColumnTableKeyList = null;
...
$(function(){
	...
	$(".f_view_log").on("click", function(){
		if (jsPopupLogDataColumnTableKeyList == undefined){
			jsPopupLogDataColumnTableKeyList = new JsPopupLogDataColumnTableKeyList(window, "#dataColumnChangedModal.modal", "JsPopupLogDataColumnTableKeyList", 1, "", "/_mng/api/popups/LogDataColumnTableKeyList.json", {});
		}

		jsPopupLogDataColumnTableKeyList.fn_loading_data_call(true, {
			"tableNm": "BPLC"
			, "originKey1":"${bplcVO.uniqueId}"
		});
...
$(document).ready(function(){
	...
	jsConsultAbleType = new JsCheckboxAll3StateList(".checkbox-group.ConsultAbleType-group", "ConsultAbleType", "${ConsltAbleType.CONSULT_ALL.getNum()}");
	jsConsultAbleType.set_checked_values(${consltAbleTypeValues});
	jsConsultAbleType.set_disabled(true);
});
</script>