	...
	var dtCall = function(){
		bplcSrchList = $("#bplcDataTable").DataTable({
				...
				{ mDataProp: "uniqueId",
					mRender: function(oObj, dp, aDt) {
						var str  = '<div class="form-check">';
							str += '<input class="form-check-input" id="modalBplc_'+aDt.uniqueId+'" name="modalBplcUniqueId" type="radio" value="'+aDt.uniqueId+'" data-bplc-nm="'+ aDt.bplcNm +'" data-bplc-id="'+ aDt.bplcId +'" data-rcmd-cnt="'+ aDt.rcmdCnt +'" data-telno="'+ aDt.telno +'" data-pictelno="'+aDt.picTelno+'">';
							str += '</div>';
				 		return str;
					}
				},
			...
			fnServerData: function ( sSource, aoData, fnCallback ) {
				...
				restParams.push({name : "srchBplcCd", value :  $("#modalSrchId").val()});
		...
		init: function(){
			...
			$(".f_modalSelBplc").on("click", function(){
				...
				if(uniqueId==null||uniqueId.length==0) {
					...
					let picTelno = $("#bplcDataTable td :radio:checked").data("pictelno");
					let rcmdCnt = $("#bplcDataTable td :radio:checked").data("rcmdCnt");
					
					f_modalBplcSearch_callback(uniqueId, bplcId, bplcNm, picTelno, rcmdCnt);
