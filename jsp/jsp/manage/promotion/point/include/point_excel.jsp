...
<%
	...
	// 엑셀 파일명 설정
	String fileName = "포인트 목록_"+EgovDateUtil.getCurrentDateAsString();
	try {
			...
			<tbody>
				<c:forEach items="${itemList}" var="result" varStatus="status">
					<tr>
						...
						<td>${result.pointSe eq 'A'?'적립':'차감'}</td>
						<td>${pointCnCode[result.pointCn]}</td>
						<td>${result.mngrMemo}</td>
						<td>${result.point}</td>
						<td><fmt:formatNumber value="${result.targetCnt}" pattern="###,###" /></td>
						<td><fmt:formatNumber value="${result.point * result.targetCnt}" pattern="###,###" /></td>
						<td>${result.rgtr}</br>(${result.regId})</td>
						<td><fmt:formatDate value="${result.regDt}" pattern="yyyy-MM-dd" /></td>
					</tr>
				</c:forEach>
				<c:if test="${fn:length(itemList) < 1 }">
					<tr>
						<td colspan="9" style="height: 50px; text-align: center; vertical-align: middle;">데이터가 없습니다.</td>
					</tr>
				</c:if>
			</tbody>
		</table>
