...
<form action="./list" method="get" id="searchFrm" name="searchFrm">
	...
	<fieldset>
		<legend class="text-title2">발행내역 검색</legend>
		<table class="table-detail">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<select class="form-control w-60" id="srchIssuTy" name="srchIssuTy" >
							<option value="">전체</option>
							<c:forEach var="issuTy" items="${issuTyCode}">
								<option value="${issuTy.key}">${issuTy.value}</option>
							</c:forEach>
					</select></td>
				</tr>
				<tr>
					...
					<td>
						<div class="form-check-group w-60">
							<div class="form-check">
								<input class="form-check-input" type="radio" name="srchSttusTy" id="srchSttusTy" checked>
								<label class="form-check-label" for="search-item5">전체</label>
							</div>
							<c:forEach var="useYn" items="${sttusTyCode}" varStatus="status">
								<div class="form-check">
									<input class="form-check-input" type="radio" name="srchSttusTy" id="srchSttusTy${status.index}" value="${useYn.key}" <c:if test="${useYn.key eq param.srchSttusTy}">checked="checked"</c:if>>
									<label class="form-check-label" for="srchSttusTy${status.index}">${useYn.value}</label>
								</div>
							</c:forEach>
						</div>
					</td>
...
<p class="text-title2 mt-13">발행내역 목록</p>
<div class="scroll-table">
	<table class="table-list">
		...
		<tbody>
			...
			<c:set var="pageParam" value="couponNo=${resultList.couponNo}&amp;curPage=${listVO.curPage}&amp;cntPerPage=${param.cntPerPage}&amp;sortBy=${param.sortBy}&amp;srchCouponTy=${param.srchCouponTy}&amp;srchCouponNm=${param.srchCouponNm}&amp;srchCouponCd=${param.srchCouponCd}
			&amp;srchIssuTy=${param.srchIssuTy}&amp;srchSttusTy=${param.srchSttusTy}&amp;srchDt=${param.srchDt}&amp;srchBgngDt=${param.srchBgngDt}&amp;srchEndDt=${param.srchEndDt}" />
				<tr>
					...
					<td class="text-left whitespace-normal"><a href="./view?${pageParam}">
						 <c:if test="${resultList.issuMbrGrad ne '' && resultList.issuMbrGrad ne NULL}">[</c:if> ${gradeCode[resultList.issuMbrGrad]}
						 <c:if test="${resultList.issuMbrGrad ne '' && resultList.issuMbrGrad ne NULL }">]</c:if> &nbsp;&nbsp; ${resultList.couponNm}
					</a></td>
					<td>${resultList.dscntAmt}<c:if test="${resultList.dscntTy eq 'PRCS'}">%</c:if>
						<c:if test="${resultList.dscntTy eq 'SEMEN'}">원</c:if><br> (<fmt:formatNumber value="${resultList.mummOrdrAmt}" pattern="0,000" />)
					</td>
					<td><fmt:formatDate value="${resultList.issuBgngDt}" pattern="yyyy-MM-dd" /> ~ <fmt:formatDate value="${resultList.issuEndDt}" pattern="yyyy-MM-dd" /></td>
					<td><c:if test="${resultList.usePdTy eq 'FIX'}">
							<fmt:formatDate value="${resultList.useBgngYmd}" pattern="yyyy-MM-dd" /> ~ <fmt:formatDate value="${resultList.useEndYmd}" pattern="yyyy-MM-dd" />
						</c:if> <c:if test="${resultList.usePdTy eq 'ADAY'}">발급일로 부터 ${resultList.usePsbltyDaycnt}일</c:if></td>
					<td><button type="button" class="cntList btn shadow w-full" data-cpn-no="${resultList.couponNo}" data-cpn-nm="${resultList.couponNm}">
							 <c:if test="${resultList.issuQy eq '9999'}">무제한</c:if>
							 <c:if test="${resultList.issuQy ne '9999'}">${resultList.issuQy}</c:if>
					</button></td>

					<td>${sttusTyCode[resultList.sttsTy]}</td>
					<td>${resultList.rgtr}</td>
					<td><fmt:formatDate value="${resultList.regDt}" pattern="yyyy-MM-dd HH:mm" /></td>
					<td>
						<c:if test="${resultList.sttsTy ne 'STOP' && resultList.issuTy ne 'AUTO' && resultList.couponTy eq 'NOR' }"><a href="./view?${pageParam}&amp;issuBtn=Y" class="btn-primary">발행</a></c:if>
					</td>
				</tr>
			</c:forEach>
