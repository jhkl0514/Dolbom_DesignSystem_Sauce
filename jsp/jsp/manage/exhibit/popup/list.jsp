...
<div id="page-content">
	<form action="./list" method="get" id="searchFrm" name="searchFrm">
		...
		<fieldset>
			<legend class="text-title2">팝업 검색</legend>
			<table class="table-detail">
				...
				<tbody>
					...
					<tr>
						...
						<td><input type="text" class="form-control w-full" id="search-item2" name="srchText" value="${param.srchText}"></td>
					</tr>
					<tr>
	...
	<p class="text-title2 mt-13">팝업 목록</p>
		...
		<tbody>
			<c:forEach items="${listVO.listObject}" var="resultList" varStatus="status">
				...
				<c:set var="pageParam" value="curPage=${listVO.curPage}${!empty(listVO.urlParam)? '&amp;' : ''}${listVO.urlParam}" />
					...
					<td class="text-left"><a href="/_mng/exhibit/popup/form?popNo=${resultList.popNo}&${pageParam}">${resultList.popSj}</a></td>
					<td><fmt:formatDate value="${resultList.bgngDt}" pattern="yyyy-MM-dd" /> ~ <fmt:formatDate value="${resultList.endDt}" pattern="yyyy-MM-dd" /></td>
	...
	<div class="btn-group right mt-8">
		<a href="/_mng/exhibit/popup/form?${pageParam}" class="btn-primary large shadow">등록</a>
	</div>
</div>
