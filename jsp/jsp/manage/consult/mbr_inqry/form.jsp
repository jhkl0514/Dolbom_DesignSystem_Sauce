...
<div id="page-content">
	...
	<fieldset class="mt-13">
		...
		<c:set var="pageParam" value="menuNo=${param.menuNo}&amp;inqryNo=${param.inqryNo}&amp;ansYn=${param.ansYn}&amp;curPage=${param.curPage }&amp;cntPerPage=${param.cntPerPage }&amp;srchRegBgng=${param.srchRegBgng}&amp;srchRegEnd=${param.srchRegEnd}
		...
		<div class="btn-group mt-8 right">
			<c:if test="${!empty mngAuthrtMenuVO && mngAuthrtMenuVO.delYn eq 'Y'}">
				<button type="button" class="btn-danger large shadow float-left mb-3  btn delete consult" id="delConslt" name="delConslt" onclick="fn_deleteInquery_call('${mbrInqryVO.getInqryNo()}')">삭제</button>
			</c:if>
...
//수급자 상담관리 삭제
function fn_deleteInquery_call(consltNo) {
	if (!confirm("1:1문의를 삭제하시겠습니까?")){
		return;
	}
	jsCallApi.call_api_post_json(window, "/_mng/consult/mbrInqry/delInqueryList.json", "fn_deleteInquery_cb", {arrDelList : [consltNo]});
}

function fn_deleteInquery_cb(result, fail, data, param) {
	alert("삭제 되었습니다.")
