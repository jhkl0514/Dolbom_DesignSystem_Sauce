				...
				<tbody>
					<tr>
						...
						<td colspan="2">
							<div class="flex flex-wrap -mx-2">
								<div class="form-group px-2 my-1 mr-auto">
									...
									<label for="member-item2-1" class="pr-1">앱푸시</label>
									<div class="form-check">
										<input type="checkbox" class="form-check-input choiceVal" name="pushRecptnYn" id="pushRecptnYn" value="${mbrVO.pushRecptnYn}" <c:if test="${mbrVO.pushRecptnYn eq 'Y' }">checked="checked"</c:if>> <label class="form-check-label" for="pushRecptnYn"></label>
									</div>
								</div>
							</div>
						</td>
						...
						<td>
							...
							${mbrVO.pushRecptnYn == 'Y' ? '동의' : '비동의'}&nbsp;<fmt:formatDate value="${mbrVO.pushRecptnDt == null ? '' : mbrVO.pushRecptnDt}" pattern="yyMMdd HH:mm:ss"/><br>
						</td>
					</tr>
					<tr>
						<th scope="row">광고성 정보 야간수신</th>
						<td colspan="2">
							<div class="flex flex-wrap -mx-2">
								<div class="form-group px-2 my-1 mr-auto">
									<label for="member-item2-1" class="pr-1">야간혜택동의</label>
									
									<div class="form-check">
										<c:set var="nightReceptionYn" value="N" />
									
										<c:if test="${ !empty mbrAgreementVO.nightReceptionYn && mbrAgreementVO.nightReceptionYn eq 'Y' }">
											<c:set var="nightReceptionYn" value="Y" />	
										</c:if>
										
										<input type="checkbox" class="form-check-input" name="nightReceptionYn" id="nightReceptionYn" value="${nightReceptionYn}" <c:if test="${nightReceptionYn eq 'Y' }">checked="checked"</c:if>> <label class="form-check-label" for="nightReceptionYn"></label>
									</div>
								</div>
							</div>
						</td>
						...
						<td>
							<c:choose>
								<c:when test="${ !empty mbrAgreementVO.nightReceptionYn }">
									${mbrAgreementVO.nightReceptionYn == 'Y' ? '동의' : '비동의'}&nbsp;<fmt:formatDate value="${mbrAgreementVO.nightReceptionDt == null ? '' : mbrAgreementVO.nightReceptionDt}" pattern="yyMMdd HH:mm:ss"/><br>
								</c:when>
								<c:otherwise>
									비동의
								</c:otherwise>
					...
					<tr>
						<td>
							<div class="form-group w-full">
								...
								<button type="button" class="btn authDirectBtn" data-bs-toggle="offcanvas" data-bs-target="#offcanvas3">직권탈퇴(바로)</button> 
...
<script>
	...
	$(function() {
		...
		$("button[type='button'].btn.authBtn").on("click", function() {
			$("#mngTyA").val("AUTH");
			$("#offcanvas3 .offcanvas-header p").html("직권탈퇴 관리")
		});
		$("button[type='button'].btn.authDirectBtn").on("click", function() { 
			$("#mngTyA").val("AUTHDIRECT"); 
			$("#offcanvas3 .offcanvas-header p").html("직권탈퇴 관리(바로)") 
		}); 
		//직권탈퇴 유효성 검사
		...
		// 야간 해택 동의
		$("#nightReceptionYn").on("click", function() {
			if (confirm("변경하시겠습니까?")) {
				var nightReceptionYn = $('#nightReceptionYn').is(':checked') ? 'Y' : 'N';
				
				$.ajax({
					type : "post",
					url : "nightReception.json",
					data : {
						nightReceptionYn,
					},
					dataType : 'json'
