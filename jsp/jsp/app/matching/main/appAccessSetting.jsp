<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
	...
	<script>
		...
		function clickConfirmBtn() {
            ...
            var requiredlogin = "${requiredlogin}";

            var url;
            if (skiponboarding == "true"){
                url = "${redirectUrl}";
            }else {
                url = "/matching/onboarding?redirectUrl=${redirectUrl}";
            }

            if (requiredlogin == "true" && url.length > 0){
                url += (url.indexOf("?")>=0?"&":"?") + "requiredlogin=" + requiredlogin;
            }

			if (isOneClick) {
				sendDataToMobileApp({ actionName: 'requestPermissions', type: 'push', redirectUrl: url });
				isOneClick = false;
			} else {
				location.href = url;
			}
