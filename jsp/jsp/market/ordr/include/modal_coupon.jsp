				...
				<div class="space-y-4 md:space-y-5">
					<c:forEach var="resultList" items="${itemList}" varStatus="status">
						<div class="order-coupon">
							...
							<button type="button" class="order-trigger ${resultList.gdsNo}_${status.index+1}" 
								data-gds-cd = "${resultList.gdsCd}" data-gds-pc = "${resultList.pc}" data-gds-no = "${resultList.gdsNo}" 
								data-dlvy-amt="${resultList.dlvyBassAmt}" data-ordr-idx="${status.index}" data-coupone-use="${resultList.getCouponUseYn()}"
							>
								<img src="" alt="" class="img${resultList.gdsNo}_${status.index+1} default">

								<c:if test="${resultList.getCouponUseYn() eq 'Y'}">
									<span class="img${resultList.gdsNo}_${status.index+1} default">적용할 쿠폰을 선택하세요</span>
								</c:if>
								<c:if test="${resultList.getCouponUseYn() ne 'Y'}">
									<span class="img${resultList.gdsNo}_${status.index+1} default">쿠폰 적용이 불가능한 상품입니다.</span>
								</c:if>
...
$(function(){
   ...
   //쿠폰사용
    $('.order-trigger').on('click', function(event) {
		...
		var couponeUse=$(this).data("coupone-use");
		if (couponeUse != 'Y'){
			return;	
		}
