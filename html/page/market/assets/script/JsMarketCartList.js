class JsMarketCartList extends JsMarketCartDrawItems{
	...
	fn_draw_cart_entrpsdlvygrp_itemgrp_one(entrpsDlvyGrpInfo, arrCartGrpBaseAllList, arrCartGrpAditAllList){
		...
		var tagSoldoutYn = 'N';
		if (json.gdsInfo.gdsTagVal != undefined && json.gdsInfo.gdsTagVal.length > 0){
			var tagList = json.gdsInfo.gdsTagVal.replaceAll(" ", "").split(',');

			if (tagList.indexOf("A") >= 0 || tagList.indexOf("B") >= 0 || tagList.indexOf("C") >= 0) {
				tagSoldoutYn = "Y";
			}
		}
		var ableBuy = ((soldoutYn=='Y' || tagSoldoutYn =='Y')?'N':'Y');
		...
		var strHtml = ''+
			'<div class="order-product-inner cartGrp{2}" cartGrpNo="{0}" {1} ctgryNo="{3}" gdsCd="{4}">'.format(json.cartGrpNo, entrpsDlvygrpAttr, entrpsDlvygrpCls, json.gdsInfo.ctgryNo, json.gdsInfo.gdsCd)+
				'<div class="order-product-item">'+
					...
					((soldoutYn == 'Y' || tagSoldoutYn =='Y') ? this.fn_draw_cart_entrpsdlvygrp_itemgrp_sold_out():'') + 
